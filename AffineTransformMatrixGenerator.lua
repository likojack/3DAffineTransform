...
local AffineTransformMatrixGenerator, parent = torch.class('nn.AffineTransformMatrixGenerator', 'nn.Module')


function AffineTransformMatrixGenerator:__init(useRotation, useScale, useTranslation)
  parent.__init(self)
...
function AffineTransformMatrixGenerator:check(input)
  if self.fullMode then
    assert(input:size(2)==9, 'Expected 9 parameters, got ' .. input:size(2))
  else
...
function AffineTransformMatrixGenerator:updateOutput(_tranformParams)
  ...
  if self.fullMode then
    ...
    if self.useScale then
      local xScale = transformParams:select(2,paramIndex)
      local yScale = transformParams:select(2,paramIndex+1)
      local zScale = transformParams:select(2,paramIndex+2)
      paramIndex = paramIndex + 3

      transformationBuffer:zero()
      transformationBuffer:select(3,1):select(2,1):copy(xScale)
      transformationBuffer:select(3,2):select(2,2):copy(yScale)
      transformationBuffer:select(3,3):select(2,3):copy(zScale)
...
function AffineTransformMatrixGenerator:updateGradInput(_tranformParams, _gradParams)
  ...
  if self.fullMode then
    ...
    self.gradInput = gradParams:view(batchSize, 9)
    ...
    if self.useTranslation then
      ...
      -- very tricky correction, it is for later part, not translation
    ...
    if self.useScale then
      ...
      local gradInputScaleparams = self.gradInput:narrow(2,paramIndex-2,3)
      local sParams = torch.Tensor(batchSize, 1, 3):typeAs(transformParams)

      sParams:select(3,1):copy(transformParams:select(2,paramIndex-2))
      sParams:select(3,2):copy(transformParams:select(2,paramIndex-1))
      sParams:select(3,3):copy(transformParams:select(2,paramIndex))
      paramIndex = paramIndex-3
      ...
      -- print(gradPa)
      gradParams:select(3,1):cmul(torch.repeatTensor(sParams:select(3,1),1,3))
      gradParams:select(3,2):cmul(torch.repeatTensor(sParams:select(3,2),1,3))
      gradParams:select(3,3):cmul(torch.repeatTensor(sParams:select(3,3),1,3))
      --original code:
