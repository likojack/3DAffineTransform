...
require 'stntd'
local M = {}
local function AffineTransformPoint()
  local pointList = - nn.Identity() -- np * 4, x y z homo
  local transMatrix = - nn.Identity() -- 3 * 4
  
  local finalOut = {pointList, transMatrix} - nn.MM(false, true) -- transpose transMatrix
  
  local gmod = nn.gModule({pointList, transMatrix}, {finalOut})
  return gmod
end

M.AffineTransformPoint = AffineTransformPoint
return M
