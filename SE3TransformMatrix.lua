local SE3TransformMatrix, parent = torch.class('nn.SE3TransformMatrix', 'nn.Module')

function SE3TransformMatrix:__init(useRotation, useScale, useTranslation)
  parent.__init(self)
...
function SE3TransformMatrix:check(input)

  if self.fullMode then

    assert(input:size(2)==9, 'Expected 7 parameters, got ' .. input:size(2))
...
function SE3TransformMatrix:updateOutput(_tranformParams)
...
--      completeTransformation = torch.bmm(completeTransformation, transformationBuffer)
    ...
    --  local scaleFactors = transformParams:select(2,paramIndex)
      ...
      transformationBuffer:select(3,3):select(2,3):add(zScale)
...
function SE3TransformMatrix:updateGradInput(_tranformParams, _gradParams)
